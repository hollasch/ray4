...
following files comprise the Ray4 ray tracer:
    ...
    Note:  All source files in this package use 8-space tabs.

Revisions:

93-02-07  Hollasch
    Minor mods to compile on the Amiga again.  I've bumped the revision to 1.30
    because the last version of this file printed out 1.25 for the revision (it
    should have been 1.00; oh well).
...
91-05-12  Hollasch
    Fixbug in shadow-ray intersection routine call; `mindist' was incorrectly
    passed by value, rather than by reference.

91-03-04  Hollasch
    Moved global variables & declarations to r4_globals.h.  Renamed r4image.h
    to r4_image.h.  Fixed error in r4_trace.c where objects behind point light
    sources could still produce shadows.

91-02-02  Hollasch
    Altered scanline buffer implementation to use a minimum sized buffer; moved
    buffer constants from ray4.h to r4_main.c

91-01-05  Hollasch
    Implemented the attribute list.  Color-rendering attributes are no longer
    embedded in each object description.  Also reversed the meaning of the
    directional light source direction:  the given vector is the direction
    _towards_ the light source.
...
90-12-13  Hollasch
    Added internal verification code for tetrahedra for debugging purposes.
    Added reflections.
